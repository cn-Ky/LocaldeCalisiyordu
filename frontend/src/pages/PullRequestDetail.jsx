...
export default function PullRequestDetail() {
  ...
  if (!data) return <Window icon="⏳" title="Yükleniyor…"><p>Yükleniyor…</p></Window>;
  ...
  return (
    ...
    >
      ...
      <div className="btn98-row" style={{ margin: '10px 0' }}>
        <Button98 onClick={() => navigate(`/projects/${project.id}/pulls`)}>← Listeye Dön</Button98>
        {pull.status === 'open' && isTargetOwner && (
          <Button98 variant="primary" onClick={merge} disabled={busy}>✅ Birleştir (Merge)</Button98>
        )}
        {pull.status === 'open' && (isTargetOwner || isAuthor) && (
          <Button98 variant="danger" onClick={close} disabled={busy}>✖ Kapat</Button98>
        )}
      </div>
      ...
      {user && (
        <div style={{ display: 'flex', gap: 6 }}>
          ...
          <Button98 onClick={sendComment}>Gönder</Button98>
        </div>
      )}
    </Window>
  );
